<?php 
 class netpluspay_model extends model{
  ...
  */
    ...
    const RETURN_URL =  'http://www.vanbedng.com/payment/feedback';

    const MERCHANT_ID = 'chVo8YhRElW23KWC';

   const WEBPAY_MERCHANT_ID = 'GTB260228469110';

   const GATEWAY_URL = 'http://netpluspay.com/testpayment/paysrc/';  

   const GATEWAY_TRANX_CHECK_URL = 'http://netpluspay.com/testpayment/payment_status.php';//'https://www.ibank.gtbank.com/GTPayService/gettransactionstatus.json';
   //https://ibank.gtbank.com/GTPay/Login.aspx';
   //https://ibank.gtbank.com/GTPayService/gettransactionstatus.xml?
   //https://ibank.gtbank.com/GTPayService/gettransactionstatus.xml

   const CURRENCY_CODE_NAIRA = 'NGN';//'566';
   ...
   function render_api(){
    ...
    $r = '<form id="netplus-pay-id" name="submit2gtpay_form" action="' . self::GATEWAY_URL . '" method="POST">';
    $r.= $this->get_config();
    $r.= '</form>';
   ...
   function init_config(){
    ...
    $this->return_url = self::RETURN_URL;
    $this->currency_code = self::CURRENCY_CODE_NAIRA;

    ///value="chVo8YhRElW23KWC">
    $this->merchant_id = self::MERCHANT_ID;
   ...
   function get_computed_hash(){
    ...
    function requeryTransaction($order_id,$transaction_id,$bank){
     
    //?bank=nppgtb&merchant_id=557444611156&transaction_id=NP160104034318000000959
     ...
     $row = $this->get_transaction_row($order_id);
    ...
    $url = self::GATEWAY_TRANX_CHECK_URL .  $r;//implode('&', $t);
     //echo $url;
    ...
    //$order_id,$payment_status,$payment_response,$transaction_id
    function update_payment_status($order_id,$payment_status,$payment_response,$transaction_id){
      //
      ...
      $post['netpluspay_transaction_id'] = $transaction_id;
      $post['transaction_id'] = $order_id;

      $post['member_id'] = $this->app->lib->auth->id;

      //print_r($this->db);

      $this->db->where(array("transaction_id"=>$order_id));
      $this->db->update('item_order',$post);

      $this->check_success($order_id,$payment_status,$payment_response);
    ...
    function check_success($transaction_id,$payment_status,$payment_response){
     ...
     $obj = $this->get_transaction_row($transaction_id);
            ...
            if ($payment_status == '00'){
               //send mail;
              //amount_paid
              if ($_REQUEST['amount_paid'] != $obj->grand_total){
                return;
              }
